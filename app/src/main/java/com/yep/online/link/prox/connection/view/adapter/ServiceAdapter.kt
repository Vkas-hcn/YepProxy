package com.yep.online.link.prox.connection.view.adapter
...
class ServiceAdapter(
    ...
    if(isAll){
        if(item.check && com.yep.online.link.prox.connection.base.App.vpnState){
            holder.aivCheck.setImageResource(R.drawable.ic_check)
            holder.llItem.background =
                holder.itemView.context.getDrawable(R.drawable.bg_check_item)
        }else{
    ...
    fun clearData(activity: ServiceActivity) {
        dataList.clear()
        activity.binding.isEmoryData = true
        notifyDataSetChanged()
    }
}